...
test('renders the contact form header', ()=> {
    ...
    expect(header).not.toHaveTextContent(/the contact form rocks/i);
});
...
test('renders ONE error message if user enters less then 5 characters into firstname.', async () => {
    ...
    // const newName = await screen.queryByText(/Joe/i);
...
test('renders ONE error message if user enters a valid first name and last name but no email.', async () => {
    ...
    userEvent.type(lastnameInput, 'Goodwin');
    userEvent.type(emailInput, '');
...
test('renders all firstName, lastName and email text when submitted. Does NOT render message if message is not submitted.', async () => {
    ...
    userEvent.type(emailInput, 'email@testemail.com');
    userEvent.click(submitButton);

    expect(screen.getByText(/Felipe/i)).toBeInTheDocument();
    expect(screen.getByText(/Slaughter/i)).toBeInTheDocument();
    expect(screen.getByText(/email@testemail.com/i)).toBeInTheDocument();
